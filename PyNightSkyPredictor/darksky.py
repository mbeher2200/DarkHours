...
_PADUS_PATH = Path(__file__).parent / "data" / "protected_areas.json.gz"
_padus_cache: list | None = None


def _load_padus_areas() -> list:
    """Load the bundled PADUS protected-areas index (generated by scripts/build_protected_areas_index.py).

    Module-level cache: loaded once per process / Lambda container.
    Returns an empty list if the data file has not been built yet.
    """
    global _padus_cache
    if _padus_cache is not None:
        return _padus_cache
    if not _PADUS_PATH.exists():
        log.debug("PADUS data file not found at %s; skipping static area lookup", _PADUS_PATH)
        _padus_cache = []
        return _padus_cache
    import gzip as _gzip
    with _gzip.open(_PADUS_PATH, "rt", encoding="utf-8") as f:
        _padus_cache = json.load(f)
    log.debug("Loaded %d PADUS protected areas from %s", len(_padus_cache), _PADUS_PATH)
    return _padus_cache
...
def _jit_geocode_candidates(
    ...
    Name resolution order (each tier is free / no extra API call):
      1. PADUS static index  — bundled national/state land dataset, works on all backends
      2. Overpass areas      — live OSM query result, local backend only
      3. _settlement()       — Nominatim or AWS Location reverse-geocode (last resort)
    """
    padus_areas = _load_padus_areas()
    seen_keys: set[tuple] = set()
    dark_clusters: list = []
    for c in candidates:
        name = _best_area_name_for_cluster(c["lat"], c["lon"], padus_areas)
        if not name and natural_areas:
...
def find_nearby(lat: float, lon: float, radius_miles:int) -> dict | None:
    ...
    # 1. Name the best_candidate separately if it exists (PADUS → Overpass → settlement)
    _padus = _load_padus_areas()
    if best_candidate:
        name = (
            _best_area_name_for_cluster(best_candidate["lat"], best_candidate["lon"], _padus)
            or _best_area_name_for_cluster(best_candidate["lat"], best_candidate["lon"], natural_areas)
            or _settlement(best_candidate["lat"], best_candidate["lon"])
        )
