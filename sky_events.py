...
def _compute_dark_hours_cycle(lat: float, lon: float, target_date: date, tz) -> list:
    ...
    for offset in range(-14, 16):
        ...
        if not sunrise:
            hours.append(0.0)
            continue
        night_start = find_event(all_events, "Night begins", after=sunset, before=sunrise)
        night_end   = find_event(all_events, "Night ends", after=night_start or sunset, before=sunrise)
