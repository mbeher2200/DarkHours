...
- **Wildcard redirect.** A second, minimal CloudFront distribution (its own `*.darkhours.app` ACM cert, no origin traffic ever reaches it) 301s any subdomain to the apex — `blog.darkhours.app/x` → `darkhours.app/blog/x`, everything else `sub.darkhours.app/x` → `darkhours.app/x`. It's separate from the main distribution because CloudFront allows only one function per (behavior, event type), and every behavior on the main distribution already has one.
