...
- **Compute.** FastAPI on AWS Lambda, shipped as a zip package (Python 3.13, arm64). CDK asset bundling pip-installs the dependencies at deploy time, so there's no application container. CloudFront fronts it, with WAFv2 rate limiting plus the AWS-managed IP-reputation, known-bad-inputs, and Core (`AWSManagedRulesCommonRuleSet`) rule groups — the Core rule set runs in COUNT, not BLOCK, until a quiet stretch of WAF logs confirms it isn't flagging legitimate traffic (e.g. uptime monitors without a `User-Agent`). Async jobs run on a second zip Lambda fed by SQS, with a dead-letter queue behind it.
...
- **Wildcard redirect.** A second, minimal CloudFront distribution (its own `*.darkhours.app` ACM cert, no origin traffic ever reaches it) 301s any subdomain to the apex — `blog.darkhours.app/x` → `darkhours.app/blog/x`, everything else `sub.darkhours.app/x` → `darkhours.app/x`. It's separate from the main distribution because CloudFront allows only one function per (behavior, event type), and every behavior on the main distribution already has one. Shares the main distribution's WebACL (no extra base/per-rule cost — WAF bills per Web ACL, not per attached distribution).
- **Response header hardening.** Two CloudFront response-headers policies: `SpaSecurityHeaders` (HSTS w/ includeSubDomains, nosniff, frame-deny, referrer-policy, a CSP scoped to what `apps/web` actually loads — Plausible, Google Fonts, the same-origin API, AWS RUM's dataplane endpoint — and a Permissions-Policy that allows only the geolocation used by "use my location") on the SPA behavior, and `BaseSecurityHeaders` (same minus CSP/Permissions-Policy) on every other behavior, since the `/blog*` origin is a separately-deployed site whose script/style needs aren't auditable from this repo.
...
- **Observability.** Structured JSON logs, a CloudWatch dashboard with metric alarms and SNS notification, and CloudWatch RUM on the frontend. See [docs/OBSERVABILITY.md](OBSERVABILITY.md).
