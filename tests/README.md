...
| `test_meteor_shower_forecast.py` | `predictor.py` | — / `eph` | `meteor_shower_forecast()`: date+coords→realistic per-shower rate (ZHR × radiant altitude × site-SQM/moonlight limiting-magnitude); `meteor_shower_forecast_range()`: best night per shower across a date range; edge cases, Perseids end-to-end smoke tests |
