...
class Cache(Protocol):
    ...
    ``set`` returns whether the value was actually stored. It is not decorative:
    the DynamoDB adapter cannot store an item over 400 KB, and a caller whose job
    is to guarantee something is cached (the TLE warmer) has to be able to tell.
    """
